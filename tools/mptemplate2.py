...
class TGenerator:
    ...
    def query(self) -> None:
        self.set_attribute("name", "Enter module name", self.name)
...
def cmd_newavr(args):
    template = TGenerator(args, t_header, t_main_embeded, [t_app_info, t_avr], [])
    #template.has_main = True
    template.generate()
    print(template)
    #template.write()
...
def main() -> None:
    ...
    parrent_parser.add_argument("--license",
                                ...
                                default=Bp.license())
    parrent_parser.add_argument(
        "--organisation", type=str, help="Name of organisation", default=Bp.organisation()
    )
